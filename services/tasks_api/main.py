...
def get_user_email(authorization: Union[str, None] = Header(None)) -> str:
...
def create_task(
    ...
    task = Task.create(uuid.uuid4(), parameters.title, user_email)
    task_store.add(task)
    return task
...
def open_tasks(
    ...
    results = [task.__dict__ for task in results]
...
def closed_tasks(
    ...
    results = task_store.list_closed(owner=user_email)
    results = [task.__dict__ for task in results]
    return APITaskList(results=results)
